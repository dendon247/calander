...
let db = JSON.parse(localStorage.getItem('cal_final_db')) || {};
...
const alphaKeys = ["1","2","3","4","5","6","7","8","9","0","Q","W","E","R","T","Y","U","I","O","P","A","S","D","F","G","H","J","K","L","DEL","Z","X","C","V","B","N","M",",",".","SPC"];
...
function renderCalendar() {
    ...
    for(let i=1; i<=totalDays; i++) {
        ...
        if(i === new Date().getDate() && m === new Date().getMonth()) cell.classList.add('today');
        ...
        // This attaches the click listener to open the modal
...
function initKB() {
    ...
    kb.innerHTML = ""; // Clear to prevent double init
    alphaKeys.forEach(k => {
        const div = document.createElement('div');
        div.className = 'k-key';
        div.innerText = k;
...
function showEntries() {
    const cont = document.getElementById('entriesList');
    cont.innerHTML = "";
    if(db[activeKey]) {
        db[activeKey].forEach((ev, idx) => {
            const row = document.createElement('div');
            row.style.cssText = `display:flex; justify-content:space-between; background:#000; padding:8px; margin-bottom:5px; border-left:3px solid ${ev.color}; border-radius:4px; font-size:0.9rem;`;
            row.innerHTML = `<span>${ev.text}</span><button onclick="delEntry(${idx})" style="color:#ff4444; background:none; border:none; cursor:pointer; font-weight:bold;">X</button>`;
...
function saveEntry() {
    ...
    localStorage.setItem('cal_final_db', JSON.stringify(db));
    renderCalendar(); 
    showEntries(); 
    document.getElementById('entryInput').value = "";
}
